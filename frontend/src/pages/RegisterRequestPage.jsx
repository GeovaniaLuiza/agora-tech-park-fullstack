...
export default function RegisterRequestPage() {
  ...
  return <AuthLayout compact><form className="auth-form register-form" onSubmit={submit} noValidate>
    ...
    <label className="terms"><input id="acceptedTerms" type="checkbox" checked={form.acceptedTerms} onChange={(e) => update('acceptedTerms', e.target.checked)} aria-invalid={Boolean(errors.acceptedTerms)} aria-describedby={errors.acceptedTerms ? 'acceptedTerms-error' : undefined} /> <span>Li e aceito os <button type="button" className="link-button terms-link" onClick={() => window.alert('Os termos de uso definem as regras para utilização da plataforma.')}>termos de uso</button> e a <button type="button" className="link-button terms-link" onClick={() => window.alert('A política de privacidade explica como os dados são tratados.')}>política de privacidade</button>.</span></label>{errors.acceptedTerms && <small id="acceptedTerms-error" className="field-error">{errors.acceptedTerms}</small>}
