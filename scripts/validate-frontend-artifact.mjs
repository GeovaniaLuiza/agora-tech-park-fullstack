...
export async function validateFrontendArtifact(directory, expectedUrl) {
  let url;
  try { url = new URL(expectedUrl); } catch { throw new Error('VITE_API_URL must be an absolute HTTPS API URL.'); }
  if (url.protocol !== 'https:' || url.username || url.password || url.search || url.hash
    || url.pathname !== '/api' || localAddress.test(url.href)) {
    throw new Error('VITE_API_URL must be a public HTTPS URL ending in /api, without credentials.');
  }
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  validateFrontendArtifact(process.argv[2] || 'frontend/dist', process.env.VITE_API_URL)
    .then(() => console.info('Frontend artifact validated: expected API URL present; no development addresses.'))
